...
%     Princeton University
    ...
    %path to basis image
    basisImagePath = 'segmentation_alignment/basis2.tif';
    
    %the width of the bounding box
    boundingWidth = 449;
    
    %the rotation angle difference betweet two asymmetric frames.
    diffAngle = 170;
    ...
    if ~isfield(parameters,'boundingWidth') || isempty(parameters.boundingWidth)
        parameters.boundingWidth = boundingWidth;
    end

    if ~isfield(parameters,'diffAngle') || isempty(parameters.diffAngle)
        parameters.diffAngle = diffAngle;
    end
