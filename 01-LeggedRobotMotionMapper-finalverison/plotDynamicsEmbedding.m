...
% Plot the trajectories on the embedding space
...
for i = 1 : length(embeddingValues)
    mkdir([save_path '\Trial' num2str(i)]);
    delete( findobj(gca, 'type', 'line') );
    for j = 1 : length(embeddingValues{i})
        fprintf(1,['Trial No.' num2str(i) 'Time ' num2str(j) '\n']);
        ...
        mk.XData = y;
        mk.YData = x;        
        filename = ['Trial' num2str(i) '_T' num2str(j)];
        set(mk,'markeredgecolor','k', 'markerfacecolor','w');
        pause(0.01)
        saveas(gcf,fullfile([save_path '\Trial' num2str(i)],filename),'jpg');
