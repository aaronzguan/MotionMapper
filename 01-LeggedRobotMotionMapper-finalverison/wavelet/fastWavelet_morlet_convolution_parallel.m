...
%   Output variables:
%
%       amp -> wavelet amplitudes (N x (numModes*numPeriods) )
...
%     Terradynamics Lab
    ...
    N = length(x);  % Total time  
    L = length(f);  % Total frequency
    amp = zeros(L,N);
    if mod(N,2) == 1
        x(end+1) = 0;
        N = N + 1;
    ...
    parfor i=1:L % loop for computing amplitude at each frenquency f
